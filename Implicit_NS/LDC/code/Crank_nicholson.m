...
rhs_u_star=zeros(size(uold));
rhs_v_star=zeros(size(vold));
if(count_star==1)
rhs_u_star(2:end-1,2:end-1)=uold(2:end-1,2:end-1)-(dt/(dx*dy))*(A_u(2:end-1,2:end-1,count_star))+(0.5*dt/(Re*dx*dy))*(D_old_u(2:end-1,2:end-1));
rhs_v_star(2:end-1,2:end-1)=vold(2:end-1,2:end-1)-(dt/(dx*dy))*(A_v(2:end-1,2:end-1,count_star))+(0.5*dt/(Re*dx*dy))*(D_old_v(2:end-1,2:end-1));
else  

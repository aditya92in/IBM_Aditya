...
x_c=0.55*L_x; x_c1=0.55*L_x;
y_c=0.6*L_y;  y_c1=0.3*L_y;
c=[x_c,y_c];
%%%%%%%%%%%%%%%
%rho=0.1;
%mu=0.01;
...
pguess=zeros(imax+2,jmax+2);% no of pressure nodes imax-1,jmax-1
pcorr=zeros(imax+2,jmax+2);
...
phi=set_phi(phi,imax,jmax,x_c,y_c,x_c1,y_c1,x,y,r1,r2);%define phi inside the solid cylinder to be 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
...
[u_star,v_star]=Adams_Bashforth(A_u,D_u,A_v,D_v,uold,vold,dt,dx,dy,Re,count_star,pnew);
...
[C_d,C_l]=integral_quant(F_x,F_y,2*r1,U_w,rho);
C_d_arr(i)=C_d;
C_l_arr(i)=C_l;
...
u_d_star=b_c_u(u_d_star,U_w,U_e,U_s,U_n);
v_d_star=b_c_v(v_d_star,V_w,V_e,V_s,V_n);
rhs(2:end-1,2:end-1)=get_rhs(u_d_star,v_d_star,dx,dy,dt);
[pcorr,err]=solve_poisson(rhs,imax,jmax,dx,dy,pcorr);
...
[unew,vnew]=correction(u_d_star,v_d_star,pcorr,dt,dx,dy);
pguess=pguess+pcorr;
%      pguess(:,1)=pguess(:,2); %dp/dx=0;
%      pguess(:,end)=-pguess(:,end-1); %P=0
%      pguess(1,:)=pguess(2,:);%dp/dy=0; south boundary
%      pguess(end,:)=pguess(end-1,:);
...
%*****************post processing
 if(mod(i,10)==0)
     disp(['residual in x= ',num2str(res_x),' residual in y=',num2str(res_y),' number of timesteps=',num2str(i),' C_d=',num2str(C_d),' C_l=',num2str(C_l),' error in poisson=',num2str(err)]);
     video_iter=video_iter+1;
% contourf(x(1:end,1:end-1),y(1:end,1:end-1),unew,'edgecolor','none');
%contourf(x(2:end-1,2:end-1),y(2:end-1,2:end-1),up);
% colorbar
% caxis([min(min(unew)) max(max(unew))])
%contourf(x,y,pnew,'edgecolor','none');
quiver(x(2:end-1,2:end-1),y(2:end-1,2:end-1),unew(2:end-1,1:end-1),vnew(1:end-1,2:end-1),2);
 hold on
 grid_plotting(x_c,y_c,x_c1,y_c1,imax,jmax,x,y,r1,r2);
 axis equal;
%contourf(x(2:end-1,2:end-1),y(2:end-1,2:end-1),pnew(2:end-1,2:end-1));hold on;colorbar;

%colorbar
%colorbar('YTick',linspace(-1.5,1.5,20));
%xlabel('x (m)');
 %ylabel('y (m)');
 title(strcat('Re-100-Velocity Vectors at t=',num2str(dt*i)))
%hold on
%grid_plotting(x_c,y_c,imax,jmax,x,y,r);

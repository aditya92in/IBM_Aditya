function [p,err]=solve_poisson(rhs,imax,jmax,dx,dy,pcorr)
...
while(err>1e-8)&&(count<20)
    %boundary conditions
      piter(:,1)=piter(:,2); %dp/dx=0;West
      piter(:,end)=-piter(:,end-1); %P=0 ;East
...
%         piter(i,j)=0.5*((piter(i,j-1)+pcorr(i,j+1)).*power(dy2,-1)+(pcorr(i+1,j)+piter(i-1,j)).*power(dx2,-1)-rhs(i,j)*power(dx2,-1)*power(dy2,-1))./(power(dx2,-1)+power(dy2,-1));
%     end
%  end

piter=adi_fourier(pcorr',rhs',dx,dy);

err=norm(pcorr(2:end-1,2:end-1)-piter(2:end-1,2:end-1));
...
pcorr=piter;
     ...
     pcorr(:,end)=-pcorr(:,end-1); %P=0
     pcorr(1,:)=pcorr(2,:);%dp/dy=0; south boundary
     pcorr(end,:)=pcorr(end-1,:);%dp/dy=0 north boundary
    p=pcorr;

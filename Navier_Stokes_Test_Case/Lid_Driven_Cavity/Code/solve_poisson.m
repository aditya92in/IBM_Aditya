function p=solve_poisson(rhs,imax,jmax,dx,dy,pnew)
...
while(err>1e-8) && (count<20)
    ...
    piter(:,end)=piter(:,end-1); %dp/dx=0 east
...
err=norm(pnew(2:end-1,2:end-1)-piter(2:end-1,2:end-1));
...
pnew=piter;
    ...
    pnew(:,end)=pnew(:,end-1); %dp/dx=0; east
    pnew(1,:)=pnew(2,:);%dp/dy=0; south boundary

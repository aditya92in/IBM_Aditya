...
for i=1:n_time_steps
    a=(i*dt);
    if(a>5)
        [x_c,y_c]=move_cyl(x_c,y_c,i*dt,dx,dy,k);
        phi=set_phi(phi,imax,jmax,x_c,y_c,x,y,r);
        phi_x=0.5*(phi(1:end,1:end-1)+phi(1:end,2:end));
        phi_y=0.5*(phi(1:end-1,1:end)+phi(2:end,1:end));
...
%if(a<5)
 if(mod(i,10)==0) &&(i>350)%&&(err<1.5e-2) 
     ...
     contourf(x(2:end-1,2:end-1),y(2:end-1,2:end-1),vort,100,'edgecolor','none');
...
title(num2str(strcat('Vorticity Cylinder(Re 40) at t*=',num2str((i*dt*U_w)/(2*r)))));
%colorbar
hold on
 grid_plotting(x_c,y_c,imax,jmax,x,y,r);
 axis equal;
...
%elseif(a>5)
   ...
   %disp(['residual in x= ',num2str(res_x),' residual in y=',num2str(res_y),' number of timesteps=',num2str(i),' C_d=',num2str(C_d),' C_l=',num2str(C_l),'error in poisson',num2str(err_pos)]);
   %quiver(x(2:end-1,2:end-1),y(2:end-1,2:end-1),unew(2:end-1,1:end-1),vnew(1:end-1,2:end-1),2);
  %title(num2str(strcat('Velocity Vectors-Moving Cylinder(Re 100) at ',num2str((i*dt*U_w)/(2*r)))));
%colorbar
   %hold on
  % grid_plotting(x_c,y_c,imax,jmax,x,y,r);
  % axis equal;
